<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id"])) {
        ...
        // Veritabanında toplamucret değerini 'ODENDI' olarak güncelle
        $updateSql = "UPDATE cihaz_kayitlar SET toplamucret = 'ÖDENDİ' WHERE id = $id";
        $baglanti->query($updateSql);

        // Başarılı güncelleme mesajı gönder
        echo "Ücret başarıyla ödendi.";
    }
?>
<!DOCTYPE html>
<!DOCTYPE html>
...
<body>
    ...
    ?><p class="hgyazisi" id="countdown"></p></strong>
                  ...
                  ?>       
                     ...
                     <div class="tablolar">
                    <input class="search" type="text" id="searchInput" onkeyup="searchTable()" placeholder="Cihaz ve Ücret Bilgisi Ara">
                ...
                <table id="table" class="table table-bordered table-striped">
                    ...
                    while ($bilgiler_row = $bilgiler_result->fetch_assoc()) {
                        ...
                        echo "<td>" . $bilgiler_row["cihazturu"] . "</td>";
                        echo "<td>" . $bilgiler_row["marka"] . "</td>";
                        echo "<td>" . $bilgiler_row["model"] . "</td>";
                        echo "<td>" . $bilgiler_row["serino"] . "</td>";
                        echo "<td>" . $bilgiler_row["faturatarihi"] . "</td>";
                        echo "<td>" . $bilgiler_row["garantibilgisi"] . "</td>";
                        echo "<td>" . $bilgiler_row["toplamucret"] . " <p class='tlyazisi'>TL</p></td>";
                        echo '<td data-id="' . $bilgiler_row["id"] . '"><button class="btn btn-success" onclick="confirmAndMarkAsPaid(' . $bilgiler_row["id"] . ')">Ödendi</button></td>';
                        echo "</tr>";
                    }
...
<script>
    // Odendi butonuna basıldığında çalışacak fonksiyon
    function markAsPaid(id) {
        // AJAX kullanarak veritabanındaki toplamucret değerini güncelle
        var xhr = new XMLHttpRequest();
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                // Güncelleme başarılı olduğunda, sayfadaki ilgili hücreyi güncelle
                var cell = document.querySelector('td[data-id="' + id + '"]');
                if (cell) {
                    cell.innerText = 'ODENDI';
                }
            }
        };
        xhr.open("GET", "ucretbildir.php?id=" + id, true);
        xhr.send();
    }

    // Odendi butonuna basıldığında onay mesajı gösteren fonksiyon
    function confirmAndMarkAsPaid(id) {
        // Kullanıcıya onay mesajı göster
        var confirmMarkAsPaid = confirm("Bu cihazın ücreti ödendi olarak işaretlensin mi?");

        // Kullanıcı onay verdiyse, işaretleme işlemini başlat
