<?php
    ...
    if (isset($_GET["id"])) {
        ...
        $silme_sql = "DELETE FROM cihaz_kayitlar WHERE id = $id";
        $silme_sonuc = $baglanti->query($silme_sql);
    
        // Silme işlemi başarılıysa 200 OK döndür
        if ($silme_sonuc) {
        ...
        // Hatalı istek durumunda 400 Bad Request döndür
...
?>    
                    ...
                    <div class="tablolar">
                    <input class="search" type="text" id="searchInput" onkeyup="searchTable()" placeholder="Cihaz Ara">
                ...
                <table id="table" class="table table-bordered table-striped">
                    ...
                    while ($bilgiler_row = $bilgiler_result->fetch_assoc()) {
                        ...
                        echo "<td>" . $bilgiler_row["saticifirma"] . "</td>";
                        echo "<td>" . $bilgiler_row["takipno"] . "</td>";
                        echo "<td>" . $bilgiler_row["garantibilgisi"] . "</td>";
                        echo "<td>" . $bilgiler_row["geneldurum"] . "</td>";
                        echo "<td>" . $bilgiler_row["sikayetariza"] . "</td>";
                        echo "<td>" . $bilgiler_row["aksesuarlar"] . "</td>";
                        echo "<td> " . $bilgiler_row["fizikselhasar"] . '</td>';
                        echo "<td><button class='btn btn-danger' onclick='confirmAndDelete(" . $bilgiler_row['id'] . ")'>Kaydı Sil</button></td>";
                        echo "</tr>";
                    }
    ...
    // AJAX kullanarak veritabanından kaydı sil
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            // Silme başarılı olduğunda, sayfadaki ilgili satırı kaldır
    ...
    xhr.open("GET", "cihazsorgula.php?id=" + id, true);
    xhr.send();
    location.reload();
}


// Onay penceresi ve kaydı silme fonksiyonunu çağıran fonksiyon
function confirmAndDelete(id) {
    // Kullanıcıya onay mesajı göster
    var confirmDelete = confirm("Bu kaydı silmek istediğinizden emin misiniz?");
    
    // Kullanıcı onay verdiyse, kaydı silme işlemini başlat
    if (confirmDelete) {
        deleteRecord(id);
    }
}
